...
export class BookingDatabaseService extends BaseService {
  ...
  async updateBookingStatus(bookingId: string, status: string): Promise<boolean> {
    try {
      ...
      // Fallback to local update if not on web
      if (!isWeb) {
        // This would require finding the right user with this booking
        // and updating it - would need more complex logic
        console.log("Local update of booking status not implemented yet");
      }
