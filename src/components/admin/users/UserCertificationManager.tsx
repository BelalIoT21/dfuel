  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="py-4">
          <h4 className="text-sm font-medium mb-2">Safety Certification</h4>
          <div className="mb-4">
            <Button
              variant={user.certifications.includes("5") ? "outline" : "default"}
              size="sm"
              className={user.certifications.includes("5") ? "bg-green-50 mr-2" : "mr-2"}
              onClick={() => handleSafetyCertification(user.id)}
              disabled={loading === 'safety' || user.certifications.includes("5")}
            >
              {loading === 'safety' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Add Safety Certificate
