...
export class MachineDatabaseService extends BaseService {
  ...
  async createMachine(machineData: MachineData): Promise<any> {
    try {
      ...
      // For native apps or when API is not available, try MongoDB first
      if (!isWeb) {
      ...
      try {
        ...
        if (response.data) {
          ...
          // If API fails, try MongoDB even for web if API is unavailable
          console.log("API failed, trying MongoDB fallback");
          const mongoMachine = await mongoMachineService.addMachine({
            _id: '', // Will be generated by MongoDB
            name: machineData.name,
            type: machineData.type,
          ...
          if (mongoMachine) {
            console.log("MongoDB fallback successful:", mongoMachine);
            return {
              id: mongoMachine._id,
              _id: mongoMachine._id,
              ...mongoMachine
        ...
        console.error("API error, trying MongoDB fallback:", apiError);
      ...
      throw new Error("Failed to create machine: No data returned from API or MongoDB");
    } catch (error) {
